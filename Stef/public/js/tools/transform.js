...
W3D.Transform = {
  ...
  init() {
    ...
    tc.addEventListener("objectChange", () => {
      ...
      if (tc.mode === "translate") {
        ...
        const source =
          tc.object === this._moveProxy ? this._moveProxy : this.selected.mesh;
    ...
    // Prepare machine action icons for selected objects
    this._initMachineActionIcons();
  },
  ...
  _bindButtons() {
    ...
    if (btnRotate)
      btnRotate.addEventListener("click", () => this.setMode("rotate"));
    if (btnDelete)
  ...
  _updateInfoStrip() {
    ...
    if (posEl)
      posEl.textContent = `X ${m.position.x.toFixed(2)}  Z ${m.position.z.toFixed(2)}`;
    if (rotEl)
      rotEl.textContent = `${THREE.MathUtils.radToDeg(m.rotation.y).toFixed(1)}°`;
  },
  ...
  _initMachineActionIcons() {
    this.actionUI = document.getElementById("machine-action-icons");
    if (!this.actionUI) return;

    this.actionUI.addEventListener("click", (e) => {
      const btn = e.target.closest(".action-icon");
      if (!btn || !this.selected) return;
      const action = btn.getAttribute("data-action");
      const url = this._getSelectedMachineLink(action);

      if (!url) {
        window.alert("Geen link beschikbaar voor deze actie.");
        return;
      }
  ...
  _getSelectedMachineLink(action) {
    if (!this.selected || !this.selected.props) return null;

    const links = {
      course:
        this.selected.props.link1 || this.selected.props.courseLink || null,
      maintenance:
        this.selected.props.link2 ||
        this.selected.props.maintenanceLink ||
        null,
      safety:
        this.selected.props.link3 || this.selected.props.safetyLink || null,
    };
  ...
  updateActionIcons() {
    ...
    if (!this.selected || !this.selected.mesh) {
      this.actionUI.classList.add("is-hidden");
      return;
    }

    const bbox = new THREE.Box3().setFromObject(this.selected.mesh);
    if (!bbox.isEmpty()) {
      ...
      this.actionUI.style.left = `${x}px`;
      this.actionUI.style.top = `${y - 8}px`;
      this.actionUI.classList.remove("is-hidden");
    } else {
      this.actionUI.classList.add("is-hidden");
    }
  },
